#!/usr/bin/env python3
"""
SkillMimic 3D Visualization - Corrected (Skeleton + Ball)

Shows complete humanoid skeleton (53 joints) with ball trajectory.
Ball features: X=53, Y=275, Z=2 (discovered via exhaustive search)
"""
...
def extract_ball_data(data: torch.Tensor) -> dict:
    ...
    # Ball position: features discovered via exhaustive search
    # X=53, Y=275, Z=2 (avg distance to hands during contact: 0.643)
...
def create_interactive_visualization(data, ball_data, joint_mapping, filepath,
                                     output_path='skillmimic_3d_corrected.html'):
    ...
    fig.update_layout(
        title=dict(
            text=f"SkillMimic: Complete Skeleton + Ball (Corrected Features)<br>"
...
def main():
    ...
    create_interactive_visualization(
        ...
        output_dir / "skillmimic_3d_corrected.html"
    )

    print("\nVisualization complete!")
    print(f"Open: {output_dir / 'skillmimic_3d_corrected.html'}")
    print("="*70)
