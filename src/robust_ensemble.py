"""
Robust ensemble with bagging to reduce variance.

Strategy:
1. Train multiple models with different random seeds
2. Average predictions to reduce variance
3. Use simpler models with more regularization
"""
...
# Conservative hyperparameters that generalize well
LGB_PARAMS = {
    "n_estimators": 80,
    "num_leaves": 8,
    "learning_rate": 0.04,
...
# Number of random seeds for bagging
N_SEEDS = 5
...
def train_robust_ensemble(data):
    """Train with bagging across multiple seeds."""
    ...
    all_models = {}  # {(pid, target, model_type, seed): model}
    all_scalers = {}
    oof_preds = np.zeros_like(y_train)

    print("\n" + "="*70)
    print(f"ROBUST ENSEMBLE WITH {N_SEEDS} SEEDS")
    print("="*70)
    ...
    for pid in unique_pids:
        ...
        for target_idx, target in enumerate(TARGETS):
            ...
            # Collect OOF predictions from all seeds
            seed_oof_preds = np.zeros((n_samples, N_SEEDS * 3))  # lgb, cat, ridge per seed
            ...
            for seed_idx, seed in enumerate(BASE_SEEDS):
                ...
                for fold, (train_idx, val_idx) in enumerate(kf.split(X_scaled)):
                    ...
                    # LightGBM
                    lgb_model = lgb.LGBMRegressor(**LGB_PARAMS, random_state=seed)
                    lgb_model.fit(X_tr, y_tr)
                    lgb_pred = lgb_model.predict(X_val)
                    seed_oof_preds[val_idx, seed_idx * 3 + 0] = lgb_pred

                    # CatBoost
                    cat_model = CatBoostRegressor(**CAT_PARAMS, random_state=seed)
                    cat_model.fit(X_tr, y_tr)
                    cat_pred = cat_model.predict(X_val)
                    seed_oof_preds[val_idx, seed_idx * 3 + 1] = cat_pred

                    # Ridge
                    ridge_model = Ridge(alpha=1.0, random_state=seed)
                    ridge_model.fit(X_tr, y_tr)
                    ridge_pred = ridge_model.predict(X_val)
                    seed_oof_preds[val_idx, seed_idx * 3 + 2] = ridge_pred

            # Average predictions across all models and seeds
            # Weight: lgb=0.4, cat=0.4, ridge=0.2 per seed
            weights = np.tile([0.4, 0.4, 0.2], N_SEEDS) / N_SEEDS
            fold_preds = np.dot(seed_oof_preds, weights)

            oof_preds[player_indices, target_idx] = fold_preds

            # Train final models on all data
            for seed_idx, seed in enumerate(BASE_SEEDS):
                lgb_final = lgb.LGBMRegressor(**LGB_PARAMS, random_state=seed)
                lgb_final.fit(X_scaled, y_target)
                all_models[(pid, target, 'lgb', seed)] = lgb_final

                cat_final = CatBoostRegressor(**CAT_PARAMS, random_state=seed)
                cat_final.fit(X_scaled, y_target)
                all_models[(pid, target, 'cat', seed)] = cat_final

                ridge_final = Ridge(alpha=1.0, random_state=seed)
                ridge_final.fit(X_scaled, y_target)
                all_models[(pid, target, 'ridge', seed)] = ridge_final
...
def predict(data, trained):
    """Generate predictions with bagging."""
    X_test = data["X_test"]
    ...
    for i, (x, pid) in enumerate(zip(X_test, test_pids)):
        ...
        for target_idx, target in enumerate(TARGETS):
            ...
            for seed in BASE_SEEDS:
                lgb_pred = models[(pid, target, 'lgb', seed)].predict(x_scaled)[0]
                cat_pred = models[(pid, target, 'cat', seed)].predict(x_scaled)[0]
                ridge_pred = models[(pid, target, 'ridge', seed)].predict(x_scaled)[0]

                preds.extend([lgb_pred, cat_pred, ridge_pred])
                weights.extend([0.4, 0.4, 0.2])

            weights = np.array(weights) / N_SEEDS
            predictions[i, target_idx] = np.dot(preds, weights)
...
def main():
    ...
    print("="*70)
    print(f"ROBUST ENSEMBLE SUBMISSION {next_num}")
    print("="*70)

    data = load_all_data()
    trained = train_robust_ensemble(data)
