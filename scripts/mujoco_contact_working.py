"""
Working MuJoCo ball-hand contact simulation.

Key fix: Properly initialize qpos for freejoint bodies.
"""
...
def simulate_shot(timeseries, keypoint_map, model, data, verbose=False):
    ...
    # Initial hand position (relative to body default at 0,0,1.5)
    init_hand = hand_pos[start_idx]
    hand_offset = init_hand - np.array([0, 0, 1.5])  # Hand joints are offsets from [0,0,1.5]

    # qpos layout:
    # [0:3] ball position
    # [3:7] ball quaternion
    # [7] hx, [8] hy, [9] hz
    data.qpos[7] = hand_offset[0]
    data.qpos[8] = hand_offset[1]
    data.qpos[9] = hand_offset[2]

    # Ball on top of hand (with slight penetration to ensure contact)
    # Hand body is at [0,0,1.5] + offset, palm top is at z + 0.02
    # Subtract 0.01 to ensure contact is established
    ball_z = 1.5 + hand_offset[2] + 0.02 + BALL_RADIUS - 0.01
    data.qpos[0] = init_hand[0]  # ball x (world coordinates)
    data.qpos[1] = init_hand[1]  # ball y (world coordinates)
    data.qpos[2] = ball_z
...
def main():
    print("=" * 80)
    print("MUJOCO BALL-HAND CONTACT (Working Version)")
    print("=" * 80)
    ...
    # Quick test - place ball slightly INTO hand to ensure contact
    print("\nQuick contact test...")
    mujoco.mj_resetData(model, data)
    # Hand top at z=1.52, ball radius=0.12, so ball center at z=1.52+0.12=1.64 for contact
    # Place slightly lower (1.63) to ensure penetration/contact
    data.qpos[0:3] = [0, 0, 1.63]  # Ball slightly penetrating hand
    data.qpos[3:7] = [1, 0, 0, 0]
    data.qpos[7:10] = [0, 0, 0]    # Hand at default [0,0,1.5]
    mujoco.mj_forward(model, data)
    print(f"  Ball at z=1.63, hand top at z=1.52")
    print(f"  Initial contact: {check_contact(model, data)}, ncon={data.ncon}")
    for _ in range(100):
        mujoco.mj_step(model, data)
    print(f"  After settle contact: {check_contact(model, data)}")
