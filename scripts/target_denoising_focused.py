"""
Target Denoising - Focused Script for LB Testing

Uses the existing standalone submissions from the original target_denoising.py run (Subs 1538, 1540, 1542)
and generates blends with Sub 2169 (current LB best) at 10/20/30%.

Also re-runs the best config (ridge_a0.5_ra10) to reproduce LOO numbers.
"""
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("TARGET DENOISING - FOCUSED REPRODUCTION + BLENDS WITH SUB 2169")
    print("=" * 70)

    # ===================================================================
    # PART 1: Use existing standalone submissions to create blends
    # ===================================================================
    print("\n" + "=" * 70)
    print("PART 1: GENERATE BLENDS WITH SUB 2169 FROM EXISTING SUBMISSIONS")
    print("=" * 70)

    sub_2169 = pd.read_csv(SUBMISSION_DIR / "submission_2169.csv")
    sub_2063 = pd.read_csv(SUBMISSION_DIR / "submission_2063.csv")

    # Old standalone submissions from the original target_denoising.py run
    standalone_subs = {
        'ridge_a0.5_ra10': pd.read_csv(SUBMISSION_DIR / "submission_1538.csv"),
        'ridge_a0.3_ra10': pd.read_csv(SUBMISSION_DIR / "submission_1540.csv"),
        'ridge_a0.2_ra10': pd.read_csv(SUBMISSION_DIR / "submission_1542.csv"),
    }

    # Check correlations between denoising subs and current best
    print("\nCorrelations between denoising standalone subs and Sub 2169:")
    for name, sub_df in standalone_subs.items():
        print(f"\n  {name} (standalone):")
        for t in TARGETS:
            col = f'scaled_{t}'
            r = np.corrcoef(sub_2169[col].values, sub_df[col].values)[0, 1]
            diff = np.mean(np.abs(sub_2169[col].values - sub_df[col].values))
            print(f"    {t}: r={r:.6f}, mean_abs_diff={diff:.6f}")

    # Generate blends: denoising at 10%, 20%, 30% with Sub 2169
    blend_weights = [0.10, 0.20, 0.30]
    generated_subs = []

    for config_name, sub_df in standalone_subs.items():
        for w in blend_weights:
            sub_num = get_next_submission_number()
            blended = sub_2169.copy()
            for t in TARGETS:
                col = f'scaled_{t}'
                blended[col] = (1 - w) * sub_2169[col].values + w * sub_df[col].values
            blended.to_csv(SUBMISSION_DIR / f"submission_{sub_num}.csv", index=False)

            desc = f"{int(w*100)}% {config_name} + {int((1-w)*100)}% Sub 2169"
            ...
            # Show per-target prediction stats
            for t in TARGETS:
                col = f'scaled_{t}'
                mean_val = blended[col].mean()
                std_val = blended[col].std()
                print(f"    {t}: mean={mean_val:.6f}, std={std_val:.6f}")

    # Also generate blends with Sub 2063 for diversity
    print("\n\nBlends with Sub 2063:")
    for config_name in ['ridge_a0.5_ra10']:  # Just the best config
        sub_df = standalone_subs[config_name]
        for w in [0.10, 0.20]:
            sub_num = get_next_submission_number()
            blended = sub_2063.copy()
            for t in TARGETS:
                col = f'scaled_{t}'
                blended[col] = (1 - w) * sub_2063[col].values + w * sub_df[col].values
            blended.to_csv(SUBMISSION_DIR / f"submission_{sub_num}.csv", index=False)

            desc = f"{int(w*100)}% {config_name} + {int((1-w)*100)}% Sub 2063"
            generated_subs.append({
                'sub_num': sub_num,
                'config': config_name,
                'blend_weight': w,
    ...
    # ===================================================================
    # PART 2: Re-run best configs to reproduce LOO numbers
    # ===================================================================
    print("\n\n" + "=" * 70)
    print("PART 2: REPRODUCE LOO NUMBERS FOR TOP 3 CONFIGS")
    print("=" * 70)
    ...
    for target in TARGETS:
        ...
        baseline_results[target] = {'oof': oof, 'test': test_pred, 'mse': mse}
        print(f"  {target}: LOO MSE = {mse:.6f}")
    baseline_mean = np.mean([baseline_results[t]['mse'] for t in TARGETS])
    print(f"  MEAN: {baseline_mean:.6f}")

    # Test 3 configs: ridge_a0.5_ra10, ridge_a0.3_ra10, ridge_a0.2_ra10
    configs = [
        ('ridge_a0.5_ra10', 0.5, 10.0),
        ('ridge_a0.3_ra10', 0.3, 10.0),
        ('ridge_a0.2_ra10', 0.2, 10.0),
    ]

    denoised_results = {}
    for config_name, blend_alpha, ridge_alpha_denoise in configs:
        print(f"\n{'=' * 70}")
        print(f"CONFIG: {config_name} (blend_alpha={blend_alpha}, ridge_alpha={ridge_alpha_denoise})")
        print(f"{'=' * 70}")
        ...
        for target in TARGETS:
            ...
            y_denoised = denoise_ridge_loo(X_train_aug, y_original, pids_train,
                                            blend_alpha=blend_alpha, ridge_alpha=ridge_alpha_denoise)
            delta = np.mean(np.abs(y_denoised - y_original))

            # Train on denoised, evaluate on ORIGINAL
            oof, test_pred = locally_weighted_prediction(
            ...
            print(f"  {target}: LOO MSE(orig)={mse_original:.6f} ({pct_change:+.1f}%), mean_shift={delta:.6f}")
        ...
        denoised_results[config_name] = {
            'mses': target_mses, 'tests': target_tests, 'mean': mean_mse
        }

        # Generate new standalone submission (to confirm it matches Sub 1538/1540/1542)
        sub_num = get_next_submission_number()
        sub = pd.DataFrame({
            'id': test_data['ids'],
            'scaled_angle': target_tests['angle'],
            'scaled_depth': target_tests['depth'],
            'scaled_left_right': target_tests['left_right'],
        })
        ...
        generated_subs.append({
            ...
            'config': config_name,
            'blend_weight': 1.0,
            'description': f'STANDALONE {config_name} (re-run)'
        })
        print(f"  Sub {sub_num}: STANDALONE {config_name} (re-run)")

        # Verify matches old submission
        old_sub_map = {'ridge_a0.5_ra10': 1538, 'ridge_a0.3_ra10': 1540, 'ridge_a0.2_ra10': 1542}
        old_sub = pd.read_csv(SUBMISSION_DIR / f"submission_{old_sub_map[config_name]}.csv")
        for t in TARGETS:
            col = f'scaled_{t}'
            max_diff = np.max(np.abs(sub[col].values - old_sub[col].values))
            print(f"    {t} vs Sub {old_sub_map[config_name]}: max_diff={max_diff:.8f}")
    ...
    for s in generated_subs:
        print(f"  Sub {s['sub_num']}: {s['description']}")

    elapsed = time.time() - t0
    print(f"\nTotal time: {elapsed:.1f}s")

    # Return the info for the report
    return {
        'baseline_mean': baseline_mean,
        'baseline_results': {t: baseline_results[t]['mse'] for t in TARGETS},
